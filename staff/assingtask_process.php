...
<?php
$query3 ="SELECT * FROM assign_task WHERE assigntask_id='$_GET[as]'";
$result3=mysqli_query($link,$query3);
$queryinfo3=mysqli_fetch_assoc($result3);
	$assignmsk_proid=$queryinfo3['project_id'];
	$assignmsk_taskid=$queryinfo3['task_ID'];
	$assignmsk_date=$queryinfo3['taskassign_createdte'];
	$assignmsk_name=$queryinfo3['taskassign_createby'];
	...
	$assignmsk_percentage=$queryinfo3['taskassign_complete_presentage'];
	$assignmsk_targetdte=$queryinfo3['taskassign_targerDate'];
...
if(isset($_POST['submit'])){
	...
	$description = $_POST['assigntask_description'];
	$presentage = $_POST['assigntask_completepresentage'];
	
		$query = "INSERT INTO `staffmaintask_progress_details` (`staffmaintask_progress_details_key`, `assignmaintaskID`, `maintaskID`, `projectID`, `staffID`, `maintaskprogress_description`, `maintaskprogress_progress`, `maintaskprogress_submit_date`, `maintaskprogress_approve_status`,`maintaskprogress_createdte`,`maintaskprogress_createby`) 
									                   VALUES (NULL, '$_GET[as]', '$assignmsk_taskid', '$assignmsk_proid', '$ses_ukey', '$description', '$presentage', '$cur_dte','0',current_timestamp(), '$ses_ukey');";
				if(mysqli_query($link,$query))
				{
					$query4 ="SELECT *
								FROM staffmaintask_progress_details 
								WHERE assignmaintaskID='$_GET[as]'
								AND maintaskID='$assignmsk_taskid'
								AND projectID='$assignmsk_proid'
								AND staffID='$ses_ukey'
								AND maintaskprogress_description='$description'
								AND maintaskprogress_progress='$presentage'
								AND maintaskprogress_submit_date='$cur_dte'";
					$result4=mysqli_query($link,$query4);
					$queryinfo4=mysqli_fetch_assoc($result4);
					 
					$subtaskprocess_key=$queryinfo4['staffmaintask_progress_details_key'];
					
					$query5 = "UPDATE `assign_task` SET taskassign_complete_presentage='$presentage' WHERE assigntask_id='$_GET[as]'";
					...
					foreach($_FILES['upd_images']['name'] as $key=>$val){
						...
						$sql29="SELECT MAX(staffmaintask_progress_image_details_key) AS maximagedetailkey FROM staffmaintask_progress_image_details";
						...
						$target_new = $target_dir ."b".$newimgkey.".".$fileType;
						if (file_exists($target_new)) {
								...
								if(move_uploaded_file($_FILES["upd_images"]["tmp_name"][$key], $target_new)){
											$awers1="b".$newimgkey.".".$fileType;
											$sql42="INSERT INTO staffmaintask_progress_image_details (staffmaintask_progress_image_details_key,maintaskprogress_ID,maintask_image_path,maintask_imgupd_createby)
																		VALUES(NULL,'$subtaskprocess_key','$awers1','$ses_ukey')";
						...
						$msg1="Dear  Coordinator\r\n"
							."A daily progress report ".$task_nme." has been submitted by ".$uadmin_first." ".$uadmin_last." at ".$cur_dte.". \r\n"
							."Thank You\r\n"
							."WorkTracker\r\n";				
						
						$ok3=mail($to,$subject,$msg1);
						if($ok3){
							$messagess="&nbsp; Sucessfully Added Progress Sub Task and Message Sent!";
							$flagmsg=1;
						}
						else{
							$messagess="&nbsp; Sucessfully Added Progress Sub Task and Message Not Sent!";
							$flagmsg=1;
						}
          ...
          <div class="card card-primary">
			...
			<form role="form" method="Post" name="f4" enctype="multipart/form-data">
				<div class="form-group">
					...
					<textarea id="assigntask_description" name="assigntask_description" class="form-control" rows="4"></textarea>
				  
				</div>
				<div class="form-group">
					<label for="projectDesc">Complete Percentage</label>
					  <input type="number" max="100" min="0" id="assigntask_completepresentage" name="assigntask_completepresentage" class="form-control" required>
				</div>
				<div class="form-group">
