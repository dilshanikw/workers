						...
						$html ='<table border="0"  width="100%">
									<tr>
										<td width="100%" >
											...
											<div style="font-size:18px;font-weight:bold;" align="center"> Tasks Progress -'. $firstname.'  '. $lastname.'</div>
...
$pdf->SetTitle('Employee Task Progress');
		...
		$html='
			...
			<table border="1" width="100%">
				...
				$result=mysqli_query($link,"SELECT * FROM assign_task INNER JOIN task ON assign_task.task_ID=task.taskID
														...
														WHERE assign_task.staff_ID='$staff'
				...
				while($test = mysqli_fetch_array($result))
				{
					...
					$html.='<tr style="font-size:10px;">';
						...
						<td width="10%"> '.$test['taskassign_Date'].'</td>
						<td width="10%"> '.$test['taskassign_targerDate'].'</td>
						<td width="10%"> '.$test['taskassign_complete_presentage'].'%</td>';
						if($test['taskassign_complete_status']==0){
							$html.='<td width="5%" align="center"> Pending</td>';
						}
						else if($test['taskassign_complete_status']==1){
							$html.='<td width="5%" align="center"> Complete</td>';
						}
						else if($test['taskassign_complete_status']==2){
							$html.='<td width="5%" align="center"> Cancel</td>';
						}
						else{
								$html.='<td width="5%" align="center"> Pending</td>';
						}
			...
			<table border="1" width="100%">
				<thead>
					<tr style="font-weight:bold;background-color:#f9f9fa;font-size:12px;">
						...
						<th width="15%"> Sub task Name</th>
						<th width="15%"> Sub task Description</th>
						<th width="10%"> Start Date</th>
						<th width="10%"> Target Date</th>
						<th width="10%"> Complete %</th>
				...
				$result2=mysqli_query($link,"SELECT * FROM assign_subtask INNER JOIN subtask ON assign_subtask.subtaskID=subtask.subtaskID
																		  INNER JOIN project ON subtask.projectID=project.projectID
																		  INNER JOIN task ON subtask.taskID=task.taskID
																		  WHERE assign_subtask.staffID='$staff'
				...
				while($test2 = mysqli_fetch_array($result2))
				{
					...
					$html.='<tr style="font-size:10px;">';
						...
						<td width="15%"> '.$test2['subtaskName'].'</td>
						<td width="15%"> '.$test2['subtaskDesc'].'</td>
						<td width="10%"> '.$test2['assign_dte'].'</td>
						<td width="10%"> '.$test2['assigntask_targetdte'].'</td>
						<td width="10%"> '.$test2['complete_presentage'].'%</td>';
						if($test2['complete_status']==0){
							$html.='<td width="10%" align="center"> Pending</td>';
						}
						else if($test2['complete_status']==1){
							$html.='<td width="10%" align="center"> Complete</td>';
						}
						else if($test2['complete_status']==2){
							$html.='<td width="10%" align="center"> Cancel</td>';
						}
						else{
								$html.='<td width="10%" align="center"> Pending</td>';
						}
...
$pdf->Output('EmployeeTaskProgress_'.$cur_dte.'_'.$staff_id.'.pdf', 'I');
