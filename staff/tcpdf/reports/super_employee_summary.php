						...
						$html ='<table border="0"  width="100%">
									<tr>
										<td width="100%" >
											...
											<div style="font-size:18px;font-weight:bold;" align="center">Staff Summary Report</div>
...
$pdf->SetTitle('Staff Summary Report');
			...
			<table border="1" width="100%">
				<thead>
					<tr style="font-weight:bold;background-color:#f9f9fa;font-size:12px;">
						<th width="5%">  #</th>
						<th width="35%"> Name With Initial</th>
						<th width="15%"> Staff ID </th>
						<th width="15%"> Total Days Worked</th>
						<th width="15%"> Total Working Hours</th>
						<th width="15%"> Total Leave Taken</th>
					</tr>
				</thead>
				<tbody>';
				$no=0;
				...
				while($test = mysqli_fetch_array($result))
				{
					...
					$html.='<tr style="font-size:10px;">';
						...
						<td width="35%"> '.$test['namewithinitials'].'</td>
						<td width="15%"> '.$test['staff_id'].'</td>';
						
						$noofdays=0;
						$result1=mysqli_query($link,"SELECT * FROM attendance
														WHERE staffID='$test[empno]'
														AND attendDate>='$startdate'
														AND attendDate<='$enddate'");
						$noofdays=mysqli_num_rows($result1);
						
						$html.='<td width="15%" align="center">'.$noofdays.'</td>';
						...
						while($test1 = mysqli_fetch_array($result1))
						{
								...
								if($test1['attendance_outdte']==null){
									$work_oneday=0;
								}
								else{
									$dteStart = new DateTime($test1['attendaneCreateDate']);
									$dteEnd   = new DateTime($test1['attendance_outdte']); 
										
									$dteDiff  = $dteStart->diff($dteEnd); 
										
									$work_oneday=$dteDiff->format("%H:%I:%S");
								
								}
							$work_timeperiod+=$work_oneday;
						}
						$html.='<td width="15%" align="center">'.$work_timeperiod.'</td>';
						
						$leave_timeperiod=0;
						$result2=mysqli_query($link,"SELECT * FROM leaves 
														WHERE staffID='$test[empno]'
														AND approvelStatus=1");
						while($test2 = mysqli_fetch_array($result2)){
							
							$acday=0;
							$diff1 = strtotime($test2['leaveEndDate']) - strtotime($test2['leaveStartDate']);
							$days = floor($diff1/(3600*24));
							
							$acday=$days+1;
							
							$leave_timeperiod+=$acday;
						}
							
						$html.='<td width="15%" align="center">'.$leave_timeperiod.'</td>';
...
$pdf->Output('StaffSummaryReport_'.$startdate.'_'.$enddate.'.pdf', 'I');
