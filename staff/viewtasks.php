            ...
            <div class="card-body">
				<table class="table  table-hover" id="taskdetails">
					...
					<tbody>
					  <?php
						$no=0;
						$result=mysqli_query($link,"SELECT * FROM assign_subtask INNER JOIN task ON assign_subtask.taskID=task.taskID
																INNER JOIN project ON assign_subtask.projectID=project.projectID
																INNER JOIN subtask ON assign_subtask.subtaskID=subtask.subtaskID
																WHERE 
																assign_subtask.staffID='$ses_ukey' AND
																task.task_status=0");
						while($test = mysqli_fetch_array($result))
						{
							$no++;			
									...
									echo"<td>"; 
											echo " <a class='btn btn-primary btn-sm' href='view_subtaskindi.php?sb=$test[subtaskID]' target='_blank'>
														  <i class='fas fa-plus'>
														  </i>
														 View Details
													</a>";
						...
						$result=mysqli_query($link,"SELECT * FROM assign_task INNER JOIN task ON assign_task.task_ID=task.taskID
																			  INNER JOIN project ON assign_task.project_id=project.projectID
																			   WHERE 
																					assign_task.staff_ID='$ses_ukey' AND
																					task.task_status=0");
						while($test = mysqli_fetch_array($result))
						{
							$no++;			
									...
									echo"<td></td>";
									echo"<td>"; 
											echo " <a class='btn btn-primary btn-sm' href='view_taskindividual.php?sb=$test[task_ID]' target='_blank'>
														  <i class='fas fa-plus'>
														  </i>
														 View Details
													</a>";
